...
PACT_UPLOAD_URL = ("https://thegrant.pactflow.io/pacts/provider/RadioBrowser/consumer/RadioBrowserClient/version")
PACT_BROKER_BASE_URL="https://thegrant.pactflow.io"
...
def pact(request):
    ...
    if not request.node.testsfailed and verify_pact:
        ...
        try:
            verify = os.system('./verify_pact.sh ' + version)  
            if verify == 0:
    ...
    iniFile = os.getcwd() + '/tests/pytest.ini'
    logPath = os.getcwd() + '/'
    config.read(iniFile)
    print('Log file = ' + logPath + config['pytest']['log_file']) 
